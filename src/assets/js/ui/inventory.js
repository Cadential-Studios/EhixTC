...
class InventoryManager {
                ...
                <div class="inventory-header bg-gray-800 rounded-lg p-4 mb-4">
                    <div class="flex flex-col lg:flex-row gap-4 items-center justify-between">
                        ...
                        <div class="filter-controls flex gap-2 flex-wrap ${this.filtersCollapsed ? 'collapsed' : ''}">
                            ...
                            <button id="comparison-toggle" 
                                    onclick="inventoryUIFeatures.toggleComparisonMode()"
                                    class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm transition-colors">
                                <i class="ph-duotone ph-scales mr-1"></i>Compare Items
                            </button>
                            
                            <button onclick="inventoryManager.clearFilters()" 
                                    class="bg-gray-600 hover:bg-gray-700 text-white px-3 py-2 rounded text-sm transition-colors">
                                <i class="ph-duotone ph-eraser mr-1"></i>Clear
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="inventory-stats mt-4">
                        <div class="grid grid-cols-2 lg:grid-cols-6 gap-4 text-sm mb-4">
                            ...
                            <button onclick="inventoryManager.toggleStatsPanel()" 
                                ...
                                <i class="ph-duotone ph-chart-bar mr-1"></i>
                                <span class="text-blue-200">Analytics</span>
                            </button>
                        </div>
                        
                        <!-- Expandable Analytics Panel -->
                        <div id="stats-panel" class="stats-panel-expanded bg-gray-700 rounded-lg p-4" style="display: none;">
    ...
    renderInventoryItem(item) {
        ...
        return `
            <div class="inventory-item ${isActivatable ? 'activatable-item' : ''} ${isSelected ? 'comparison-selected' : ''} bg-gray-700 border-2 ${rarityBorderColor} rounded-lg p-3 mb-2 cursor-pointer hover:bg-gray-600 transition-all duration-200 ${!canUse ? 'opacity-60' : ''}"
                 data-inventory-item-id="${item.id}"
                 onclick="${isComparisonMode ? '' : `inventoryManager.openItemModal('${item.id}')`}">
                ${isActivatable ? `
                ...
                <!-- Drag handle -->
                <div class="drag-handle absolute top-2 right-2 opacity-50 hover:opacity-100">
                    <i class="ph-duotone ph-dots-six text-gray-400"></i>
                </div>
                ...
                <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="flex flex-col items-end text-xs text-gray-400">
                        ...
                        ${item.data.weight ? `<span class="text-gray-400">${item.data.weight}lb</span>` : ''}
                    </div>
                </div>
    ...
    useItem(itemId) {
        ...
        // Handle consumable items using the new consumable system
        if (item.type === 'consumable') {
            if (typeof useConsumableItem === 'function') {
                const success = useConsumableItem(itemId, 1);
                if (success) {
        ...
        container.innerHTML = `
            <div class="analytics-dashboard">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    <!-- Overview Card -->
                    <div class="analytics-card bg-gray-600 rounded-lg p-4">
                        <h4 class="text-lg font-semibold text-white mb-3 flex items-center">
                            <i class="ph-duotone ph-chart-pie mr-2 text-blue-400"></i>
                            Overview
                        </h4>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-300">Total Items:</span>
                                <span class="text-white font-semibold">${stats.overview.totalItems}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-300">Unique Items:</span>
                                <span class="text-white font-semibold">${stats.overview.uniqueItems}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-300">Total Value:</span>
                                <span class="text-green-400 font-semibold">${stats.overview.totalValue}g</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-300">Encumbrance:</span>
                                <span class="text-${stats.overview.encumbrancePercentage > 80 ? 'red' : stats.overview.encumbrancePercentage > 60 ? 'yellow' : 'green'}-400 font-semibold">
                                    ${stats.overview.encumbrancePercentage}%
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="analytics-card bg-gray-600 rounded-lg p-4">
                        <h4 class="text-lg font-semibold text-white mb-3 flex items-center">
                            <i class="ph-duotone ph-stack mr-2 text-green-400"></i>
                            Item Types
                        </h4>
                        <div class="space-y-2 text-sm">
                            ${Object.entries(stats.breakdown).map(([type, data]) => `
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-white font-semibold">${data.count}</span>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                    ...
                    <div class="analytics-card bg-gray-600 rounded-lg p-4">
                        <h4 class="text-lg font-semibold text-white mb-3 flex items-center">
                            <i class="ph-duotone ph-star mr-2 text-yellow-400"></i>
                            Rarity Spread
                        </h4>
                        <div class="space-y-2 text-sm">
                            ${Object.entries(stats.rarity).filter(([rarity, data]) => data.count > 0).map(([rarity, data]) => `
                                <div class="flex justify-between">
                                    <span class="text-gray-300 capitalize">${rarity}:</span>
                                    <span class="text-white font-semibold">${data.count} (${data.percentage}%)</span>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ...
                ${stats.recommendations.length > 0 ? `
                    <div class="recommendations mb-6">
                        <h4 class="text-lg font-semibold text-white mb-3 flex items-center">
                            <i class="ph-duotone ph-lightbulb mr-2 text-yellow-400"></i>
                            Recommendations
                        </h4>
                        <div class="space-y-3">
                            ${stats.recommendations.map(rec => `
                                <div class="recommendation-item bg-gray-600 rounded-lg p-3 border-l-4 border-${
                                    rec.type === 'warning' ? 'red' : 
                                    rec.type === 'caution' ? 'yellow' : 'blue'
                                }-500">
                                    <div class="flex items-start">
                                        <i class="ph-duotone ph-${
                                            rec.type === 'warning' ? 'warning' : 
                                            rec.type === 'caution' ? 'warning-octagon' : 'info'
                                        } text-${
                                            rec.type === 'warning' ? 'red' : 
                                            rec.type === 'caution' ? 'yellow' : 'blue'
                                        }-400 mr-3 mt-1"></i>
                                        <div>
                                            <div class="font-semibold text-white">${rec.title}</div>
                                            <div class="text-gray-300 text-sm">${rec.message}</div>
                                            <div class="text-${
                                                rec.type === 'warning' ? 'red' : 
                                                rec.type === 'caution' ? 'yellow' : 'blue'
                                            }-400 text-sm font-medium mt-1">${rec.action}</div>
                                        </div>
                                    </div>
                                </div>
                            `).join('')}
                ...
                <!-- Value Analysis -->
                <div class="value-analysis mb-6">
                    <h4 class="text-lg font-semibold text-white mb-3 flex items-center">
                        <i class="ph-duotone ph-coins mr-2 text-green-400"></i>
                        Most Valuable Items
                    </h4>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div class="bg-gray-600 rounded-lg p-3">
                            ${stats.value.top5Items.map(item => `
                                <div class="flex justify-between text-sm mb-1">
                                    <span class="text-gray-300">${item.name}</span>
                                    <span class="text-green-400 font-semibold">${item.value}g ${item.quantity > 1 ? `(x${item.quantity})` : ''}</span>
                                </div>
                            `).join('')}
                        </div>
                        
                        ${stats.efficiency.valuePerWeight.length > 0 ? `
                            <div class="bg-gray-600 rounded-lg p-3">
                                <div class="text-sm font-semibold text-gray-200 mb-2">Best Value/Weight Ratio:</div>
                                ${stats.efficiency.valuePerWeight.slice(0, 5).map(item => `
                                    <div class="flex justify-between text-sm mb-1">
                                        <span class="text-gray-300">${item.name}</span>
                                        <span class="text-blue-400 font-semibold">${item.ratio.toFixed(1)} g/lb</span>
                                    </div>
                                `).join('')}
                            </div>
                        ` : ''}
